    ...
    <Card className="px-2 pt-2 sm:p-4 rounded-lg border-none" style={{boxShadow: "0 10px 25px rgba(0,0,0,0.1)"}}>
      ...
      <CardContent className="p-0 m-0">
        <div className="flex justify-between gap-1 items-center">
            <h3 className="text-gray-700 text-sm sm:text-xl sm:font-medium md:text-2xl lg:text-xl lg:tracking-normal 2xl:text-2xl">{product.name}</h3>
            <div className="flex items-center space-x-1">
            {Array.from({ length: 5 }, (_, i) => (
                <Star
                key={i}
                className={`w-2 h-2 sm:w-3 sm:h-3 xl:h-3.5 xl:w-3.5 2xl:w-4 2xl:h-4 ${i < product.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                />
            ))}
            </div> 
        </div>
      ...
      <CardFooter className="flex justify-between items-center px-0 -my-2 sm:mb-1 xl:mb-3 2xl:mb-6">
        <p className="text-md sm:text-lg xl:text-xl font-medium text-gray-800 2xl:text-2xl">${product.price.toFixed(2)}</p>
