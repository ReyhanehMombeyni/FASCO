...
import { priceRanges } from "@/src/constants";
import { useFilterUpdater } from "@/src/hooks/useFilterUpdater";
import { ItemsFilter } from "@/src/types/shop";
...
const FilterSection = ({
  ...
  return (
    <div className="mb-2 pb-2 lg:pb-5">
      <div
        className="flex justify-between items-center text-xs md:text-sm lg:text-base xl:text-lg mb-1 cursor-pointer hover:text-gray-800"
        onClick={toggleOpen}
      >
        <h3 className="sm:tracking-wider">{title}</h3>
        <ChevronUp
          className={`w-2.5 h-2.5 md:w-3 md:h-3 lg:w-4 lg:h-4 transition-transform ${
...
export function FilterSidebar({ itemsFilter }: { itemsFilter: ItemsFilter }) {
  ...
  return (
    <div className="space-y-0 font-serif">
      <h2 className="text-sm md:text-lg md:tracking-wide lg:text-xl xl:text-2xl mb-2">
        Filters
      </h2>
          ...
          {sizes.map(({ id, name, symbol }) => {
            ...
            return (
              <Button
                ...
                className={`w-3 h-3 p-1.5 text-[6px] sm:text-[8px] sm:p-2 md:text-[10px] md:p-2.5 xl:text-xs xl:p-3 rounded-[2px] md:rounded-xs xl:rounded-sm border ${
                  isActive
            ...
            return (
              <div
                ...
                className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 2xl:w-7 2xl:h-7 rounded-full cursor-pointer border border-gray-200 transition-all ${
                  isActive ? "border border-gray-600 shadow-2xs" : ""
                }`}
          ...
          {priceRanges.map(({ id, min_range, max_range }, index) => {
            ...
            return (
              <div
                ...
                className={`text-[8px] sm:text-[10px] md:text-xs sm:py-0.5 lg:py-1 lg:text-sm 2xl:text-base cursor-pointer ${
                  isActive
          ...
          {tags.map(({ id, name }) => {
            ...
            return (
              <span
                ...
                className={`${
                  isActive && "text-gray-950 font-black"
                } text-[8px] px-1 sm:text-[10px] xl:text-[11px] md:py-1 md:px-2 2xl:px-3 2xl:text-xs 2xl:py-1.5 bg-gray-100 text-gray-700 rounded-full cursor-pointer hover:bg-gray-200`}
              >
                {name}
              </span>
            );
